...
/**
 * Детектор шифра Трифид.
 *
 * IoC ещё ниже Bifid (более рассеянное распределение).
 */
final readonly class TrifidDetector implements CipherDetectorInterface
{
    /**
     * {@inheritDoc}
     */
    public function detect(IdentificationContext $ctx): ?CipherDetection
    {
        ...
        $upperBound = $naturalIoc - ($naturalIoc - $randomIoc) * 0.4;
        if ($iocValue >= $upperBound || $iocValue < $randomIoc * 0.8) {
            return null;
        }

        return new CipherDetection(
            toolSlug: 'classical-ciphers/trifid',
            cipherKey: 'CIPHER_NAME_TRIFID',
            confidence: 0.40,
