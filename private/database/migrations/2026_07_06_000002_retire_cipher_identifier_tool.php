...
/**
 * Ретайрит инструмент «Cipher Identifier»: его функциональность поглощена
 * флагманским хабом «Cipher Solver» (/cipher-solver).
 *
 * Удаляет каталожную запись инструмента из категории «Анализ текста» вместе со
 * связанным контентом и регистрирует 301-редиректы со старого URL инструмента
 * на URL хаба для всех локалей. API-эндпоинт /api/tools/cipher-identifier
 * сохраняется — им продолжает пользоваться сам солвер и внешние интеграции.
 */
class RetireCipherIdentifierTool extends Migration
{
    /**
     * Выполняет ретайр: удаление инструмента и сид редиректов.
     */
    ...
    /**
     * Откатывает миграцию: удаляет редиректы (каталожная запись не восстанавливается).
     */
    ...
    /**
     * Удаляет каталожную запись инструмента cipher-identifier и весь связанный контент.
     */
    ...
    /**
     * Регистрирует 301-редиректы со старого URL инструмента на URL хаба по всем локалям.
     */
    ...
    private function redirectPairs(): array
    {
        ...
        foreach ($locales as $locale) {
            ...
            $pairs[] = [
                $prefix . '/text-analysis/cipher-identifier',
