...
6. Добавление новых объектов (п.5) разрешено только когда `meta.language == meta.default_language` (например, `en`). Для остальных языков нужно переводить уже существующие `id`.
7. Для `blocks[].data.text` используйте HTML-разметку: минимум один `<p>`, можно несколько `<p>`, также допустимы списки `<ul>/<ol>` с `<li>`.
